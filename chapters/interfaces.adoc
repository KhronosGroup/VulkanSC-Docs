...
[[interfaces-explicit-layout-decorations]]
...
ifdef::VK_COMPUTE_VERSION_1_1,VK_KHR_16bit_storage[]
...
endif::VK_COMPUTE_VERSION_1_1,VK_KHR_16bit_storage[]
...
<<interfaces-iointerfaces-user,User-defined variables>> in interfaces
between shader stages in the graphics pipeline consume a unique set of
code:Location and code:Component values.

Available space for user-defined interface variables is partitioned into a
number of 32-bit four-component vectors, each identified by a code:Location
value.
Each individual 32-bit component of a vector is then further identified by a
code:Component value.

ifdef::VK_COMPUTE_VERSION_1_1,VK_KHR_16bit_storage[]
16-bit scalar or vector values consume one code:Component slot per 16-bit
component and must: be specified within a single code:Location.
endif::VK_COMPUTE_VERSION_1_1,VK_KHR_16bit_storage[]
32-bit scalar or vector values consume one code:Component slot per 32-bit
component and must: be specified within a single code:Location.
64-bit scalar or vector values consume two consecutive code:Component slots
per 64-bit component from up to two consecutive code:Location slots.

For any shader interface variable where <<interfaces-iointerfaces-matching,
one level of the array is disregarded for type matching>>, the outer array
level is also disregarded when assigning code:Location slots.

An array of size [eq]#n# with elements consuming [eq]#l# code:Location slots
each will consume [eq]#l {times} n# code:Location slots.
Each element of the array will consume code:Component slots in each
code:Location slot identically to a declaration using the element type.

[NOTE]
====
Applications running on
ifndef::VKSC_VERSION_1_0[Vulkan]
ifdef::VKSC_VERSION_1_0[Vulkan SC]
implementations advertising a
slink:VkPhysicalDeviceDriverProperties::pname:conformanceVersion
ifndef::VKSC_VERSION_1_0[less than 1.4.6.0]
ifdef::VKSC_VERSION_1_0[less than or equal to 1.0.2.1]
should be aware that arrays may consume all four code:Component slots in
each code:Location slot.
====

Matrices of size [eq]#n {times} m# are assigned locations identically to
arrays of size [eq]#n# of vectors of length [eq]#m# with an identical
element type.

When a variable with a structure type is decorated with a code:Location, the
members in the structure type must: not be decorated with a code:Location.
...
The code:Location slots consumed by structure members are determined by
...
A variable with a structure type that is not decorated with code:Block must:
be decorated with a code:Location.

When a variable with a structure type decorated with code:Block is declared
without a code:Location decoration, each member in the structure must: be
...
overlapping code:Component slots within the same code:Location.
...
Multiple <<interfaces-iointerfaces-user,user-defined input variable>>
declarations must: not have overlapping code:Component slots within the same
code:Location.
...
[open,refpage='vkCmdSetRenderingAttachmentLocations',desc='Set color attachment location mappings for a command buffer',type='protos']
--
...
[open,refpage='VkRenderingAttachmentLocationInfo',desc='Structure specifying attachment locations',type='structs']
--
...
slink:VkCommandBufferBeginInfo::pname:flags, members of this structure are
ignored.
If this structure is not included in the pname:pNext chain of
...
attachments writes.
If an output variable is declared, but there is no color attachment at that
code:Location, any values written to it are discarded.
If a color attachment at a given code:Location does not have an output
variable declared, the values must: not be modified.
...
the same <<formats-numericformat, numeric type>>.
...
code:Component slots within the same code:Location and with the same
code:Index value as any other output variable declaration.
...
[[interfaces-tile-attachment]]
== Tile Attachment Interface

The image variables declared with code:TileAttachmentQCOM storage class form
the _tile attachment interface_.

These <<renderpass-tile-shading-attachment-access,tile attachment
variables>> correspond to a per-tile view of the color, depth, or input
...
<<renderpass-tile-shading, enables tile shading>>.
Access of such variables in a fragment shader, additionally requires that
the <<features-tileShadingFragmentStage,tileShadingFragmentStage>> feature
must: be enabled.

Tile attachment variables must: not include a code:Component decoration.
Tile attachment variables must: not be consumed by code:OpImageQuery*
instructions.
Tile attachment variables can: be declared as either single-sampled with
code:MS operand of `0`, or as multi-sampled with code:MS operand of `1`.
The image subresources of the tile attachment image must: not be in
ename:VK_IMAGE_LAYOUT_UNDEFINED
ifdef::VK_EXT_attachment_feedback_loop_layout[]
or ename:VK_IMAGE_LAYOUT_ATTACHMENT_FEEDBACK_LOOP_OPTIMAL_EXT
...
slink:VkImageView in the slink:VkFramebuffer
ifdef::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[]
or the slink:VkRenderingAttachmentInfo
endif::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[]
except for pname:subresourceRange.aspectMask.
The pname:aspectMask must: be equal to the aspect accessed by the shader.

_Tile attachment variables_ are further subdivided into _storage tile
attachment_, _sampled tile attachment_, and _input tile attachment_
variables.

  * _Sampled tile attachment_ variables must: be declared with a
    code:Sampled operand of `1`, must: be backed by a descriptor of type
...
ifdef::VK_QCOM_image_processing[]
    ...
    or ename:VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, and can: be used
    with code:OpImageFetch, code:OpImageSparseFetch, or used to construct an
    code:OpTypeSampledImage that is subsequently consumed by
    code:OpImageSample*, code:OpImageSparseSample*,
ifdef::VK_QCOM_image_processing[]
    code:OpImageSampleWeightedQCOM, code:OpImageBoxFilterQCOM,
    code:OpImageBlockMatch*QCOM,
endif::VK_QCOM_image_processing[]
    code:OpImage*Gather,
ifdef::VK_QCOM_image_processing3[code:OpImageGatherQCOM,]
    or code:OpImageSparse*Gather.
    _Sampled tile attachment_ variables are managed by the
    <<interfaces-resources-descset,Descriptor Set Interface>> as sampled
    images.
  * _Storage tile attachment_ variables must: be declared with a
    code:Sampled operand of `2`, must: be backed by a descriptor of type
    ename:VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, and can: be used with
    code:OpImageRead, code:OpImageSparseRead, and code:OpImageWrite
    instructions.
    _Storage tile attachment_ variables can: be consumed by
    code:OpImageTexelPointer
ifdef::VK_KHR_shader_untyped_pointer+VK_EXT_descriptor_heap[]
    and code:OpUntypedImageTexelPointerEXT
endif::VK_KHR_shader_untyped_pointer+VK_EXT_descriptor_heap[]
    for compatibility with atomic operations.
    _Sampled tile attachment_ variables are managed by the
    <<interfaces-resources-descset,Descriptor Set Interface>> as storage
    images.
  * _Input tile attachment_ variables must: be declared with a code:Sampled
    operand of `2`, must: be backed by a descriptor of type
    ename:VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, and can: be used with
    code:OpImageRead instructions.
    _Input tile attachment_ variables are managed by the
    <<interfaces-resources-descset,Descriptor Set Interface>> as input
    attachment images.

Tile attachment access using code:OpImageWrite instructions must: be used
only in the compute stage.
Tile attachment access using code:OpImageWrite instructions must: not be
used on a variable whose underlying descriptor references the same
slink:VkImageView bound as a depth or stencil attachment.
...
equal to slink:VkDescriptorSetLayoutBinding::pname:descriptorCount.
endif::VK_COMPUTE_VERSION_1_2,VK_EXT_descriptor_indexing[]
...
[open,refpage='vkCmdSetRenderingInputAttachmentIndices',desc='Set input attachment index mappings for a command buffer',type='protos']
--
...
[open,refpage='VkRenderingInputAttachmentIndexInfo',desc='Structure specifying input attachment indices',type='structs']
--
...
****
  ...
  * [[VUID-VkRenderingInputAttachmentIndexInfo-pDepthInputAttachmentIndex-12274]]
    Elements of pname:pDepthInputAttachmentIndex,
    pname:pStencilInputAttachmentIndex, and
    pname:pColorAttachmentInputIndices that are not
    ename:VK_ATTACHMENT_UNUSED must: be less than
...
See <<descriptors-inputattachment>> for more details.
...
[[interfaces-ray-pipeline-shader-interface-table]]
...
Shaders can: access <<resources, resources>> and <<samplers, samplers>>
through <<descriptors,Resource Descriptors>> created in the API, via the
ifdef::VK_EXT_descriptor_heap[]
<<interfaces-resources-descriptorheap, Descriptor Heap Interface>> or the
endif::VK_EXT_descriptor_heap[]
<<interfaces-resources-descset, Descriptor Set Interface>>.
...
ifdef::VK_QCOM_tile_shading[]
code:TileAttachmentQCOM,
endif::VK_QCOM_tile_shading[]
ifdef::VK_COMPUTE_VERSION_1_1,VK_KHR_storage_buffer_storage_class[]
code:StorageBuffer,
endif::VK_COMPUTE_VERSION_1_1,VK_KHR_storage_buffer_storage_class[]
or code:PushConstant.
For the fragment shader, this includes the <<interfaces-inputattachment,
fragment input attachment interface>>.

The shader resource interface is made up of the sub-interfaces defined in
...
block must: be placed at an code:Offset such that the entire member is
entirely contained within the range specified by flink:vkCmdPushDataEXT in
the command buffer.
...
[eq]#[0, pname:maxPushDataSize)#.
endif::VK_EXT_descriptor_heap[]

When using
ifdef::VK_EXT_descriptor_buffer[descriptor buffers or]
descriptor sets, each statically used member of a push constant block must:
be placed at an code:Offset such that the entire member is entirely
contained within the slink:VkPushConstantRange for each code:OpEntryPoint
that uses it, and the pname:stageFlags for that range must: specify the
appropriate elink:VkShaderStageFlagBits for that stage.
The code:Offset decoration for any member of a push constant block must: not
cause the space required for that member to extend outside the range
...
<<shaders-pushconstant-decorations-banknv, code:BankNV>> and
...
The code:BankNV decoration specifies which hardware bank the push constant
data should be placed in or accessed from, while code:MemberOffsetNV
...
ifdef::VK_EXT_descriptor_heap[]
[[interfaces-resources-descriptorheap]]
=== Descriptor Heap Interface

The _descriptor heap interface_ provides access to resources through
pointers to heaps of different types, without many of the restrictions that
apply to the <<interfaces-resources-descset, descriptor set interface>>.

Two built-in pointers are available to shaders:

  * <<SamplerHeapEXT,code:SamplerHeapEXT>> for <<samplers,samplers>>
  * <<ResourceHeapEXT,code:ResourceHeapEXT>> for <<resources,resources>>

These built-ins must: be declared as pointers in the code:UniformConstant
{StorageClass}.
...
The code:UniformId decoration can: be applied to the result of accesses to
...
[[interfaces-resources-heap-type-correspondence]]
.Heap Resource Type Correspondence
[width="100%",cols="20,10,10",options="header"]
|====
| Descriptor Type                               | Heap                 | SPIR-V Type
| ename:VK_DESCRIPTOR_TYPE_SAMPLER                    | code:SamplerHeapEXT  | code:OpTypeSampler
| ename:VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE              | code:ResourceHeapEXT | code:OpTypeImage
| ename:VK_DESCRIPTOR_TYPE_STORAGE_IMAGE              | code:ResourceHeapEXT | code:OpTypeImage
| ename:VK_DESCRIPTOR_TYPE_SAMPLE_WEIGHT_IMAGE_QCOM   | code:ResourceHeapEXT | code:OpTypeImage
| ename:VK_DESCRIPTOR_TYPE_BLOCK_MATCH_IMAGE_QCOM     | code:ResourceHeapEXT | code:OpTypeImage
| ename:VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT           | code:ResourceHeapEXT | code:OpTypeImage
| ename:VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER       | code:ResourceHeapEXT | code:OpTypeImage
| ename:VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER       | code:ResourceHeapEXT | code:OpTypeImage
| ename:VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER             | code:ResourceHeapEXT | code:OpTypeBufferEXT with the code:Uniform {storageclass}
| ename:VK_DESCRIPTOR_TYPE_STORAGE_BUFFER             | code:ResourceHeapEXT | code:OpTypeBufferEXT with the code:StorageBuffer {storageclass}
ifdef::VK_KHR_acceleration_structure[]
| ename:VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR | code:ResourceHeapEXT | code:OpTypeAccelerationStructureKHR
endif::VK_KHR_acceleration_structure[]
ifdef::VK_NV_ray_tracing[]
| ename:VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_NV  | code:ResourceHeapEXT | code:OpTypeAccelerationStructureNV
endif::VK_NV_ray_tracing[]
ifdef::VK_NV_partitioned_acceleration_structure[]
| ename:VK_DESCRIPTOR_TYPE_PARTITIONED_ACCELERATION_STRUCTURE_NV | code:ResourceHeapEXT | code:OpTypeAccelerationStructureKHR
endif::VK_NV_partitioned_acceleration_structure[]
ifdef::VK_ARM_tensors[]
| ename:VK_DESCRIPTOR_TYPE_TENSOR_ARM                 | code:ResourceHeapEXT | code:OpTypeTensorARM
...
a number of bytes equal to value advertised for the elink:VkDescriptorType
as returned by flink:vkGetPhysicalDeviceDescriptorSizeEXT.

For image types, there are further restrictions on the operands used for the
type, according to the descriptor type:

[[interfaces-resources-heap-image-operand-correspondence]]
.Heap Image Operand Correspondence
[width="100%",cols="20,8,8,8,8",options="header"]
|====
| Descriptor Type                               | code:Dim         | code:Arrayed | code:MS | code:Sampled
| VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE              | *                | *            | 0       | 1
| VK_DESCRIPTOR_TYPE_STORAGE_IMAGE              | *                | *            | 0 or 1  | 2
| VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT           | code:SubpassData | 0            | 0 or 1  | 2
| VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER       | code:Buffer      | 0            | 0       | 2
| VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER       | code:Buffer      | 0            | 0       | 2
| VK_DESCRIPTOR_TYPE_SAMPLE_WEIGHT_IMAGE_QCOM   | code:2D          | 1            | 0       | 1
| VK_DESCRIPTOR_TYPE_BLOCK_MATCH_IMAGE_QCOM     | code:2D          | 0            | 0       | 1
|====

For storage images and input attachments, code:MS is 0 if the image has one
sample per pixel, or 1 otherwise.
For sampled and storage images, the code:Dim and code:Arrayed qualifiers
depend on the elink:VkImageViewType specified when writing the descriptor:

[[interfaces-resources-heap-image-dim-correspondence]]
.Heap Image View Type Dimensionality
[width="100%",cols="10,10,10",options="header"]
|====
| Image View Type               | code:Dim  | code:Arrayed
| VK_IMAGE_VIEW_TYPE_1D         | code:1D   | 0
| VK_IMAGE_VIEW_TYPE_2D         | code:2D   | 0
| VK_IMAGE_VIEW_TYPE_3D         | code:3D   | 0
| VK_IMAGE_VIEW_TYPE_CUBE       | code:Cube | 0
| VK_IMAGE_VIEW_TYPE_1D_ARRAY   | code:1D   | 1
| VK_IMAGE_VIEW_TYPE_2D_ARRAY   | code:2D   | 1
| VK_IMAGE_VIEW_TYPE_CUBE_ARRAY | code:Cube | 1
|====

The <<spirvenv-format-type-matching, type>> and <<spirvenv-image-formats,
format>> of the image resource must: also match between the API and SPIR-V.

Descriptors accessed via the code:ResourceHeapEXT built-in must: be
<<interfaces-resources-layout, explicitly laid out>>.
...
heaps, but may: be accessed by specifying <<descriptorheaps-bindings, Shader
Bindings>> to map to the descriptor heaps.
endif::VK_EXT_descriptor_heap[]
...
ifndef::VK_COMPUTE_VERSION_1_1,VK_KHR_storage_buffer_storage_class[]
...
ifdef::VK_QCOM_tile_shading[]
_Storage tile attachment_ and _sampled tile attachment_ variables declared
as described in the <<renderpass-tile-shading-attachment-access, tile
attachment interface>> are also managed by this interface.
The requirements in this section for storage image variables also applies to
_storage tile attachment_ variables.
The requirements in this section for sampled image variables also applies to
_sampled tile attachment_ variables.
The requirements in this section for input attachment variables also applies
to _input tile attachment_ variables.
endif::VK_QCOM_tile_shading[]
...
<<descriptors-combinedimagesampler, combined image sampler descriptor>> can:
have a type of code:OpTypeImage, code:OpTypeSampler (code:Sampled=1), or
...
type, but shader variables of code:OpTypeSampledImage can be mapped to a
...
ifdef::VK_QCOM_image_processing[]
  * <<descriptors-weightimage,weight image>>:
  ** Dynamically uniform: Always supported.
  ** Non-uniform: Never supported.
  * <<descriptors-blockmatch, Block matching image>>:
  ** Dynamically uniform: Always supported.
  ** Non-uniform: Never supported.
endif::VK_QCOM_image_processing[]
ifdef::VK_ARM_tensors[]
  * Storage tensors:
  ** Dynamically uniform: pname:shaderStorageTensorArrayDynamicIndexing and
     code:StorageTensorArrayDynamicIndexingARM
  ** Non-uniform: pname:shaderStorageTensorArrayNonUniformIndexing and
...
ifdef::VK_COMPUTE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion,VK_EXT_fragment_density_map[]
...
endif::VK_COMPUTE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion,VK_EXT_fragment_density_map[]
...
ifdef::VK_EXT_descriptor_heap[]
                           or both ename:VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE and
                           ename:VK_DESCRIPTOR_TYPE_SAMPLER when using heaps
endif::VK_EXT_descriptor_heap[]
...
ifdef::VK_ARM_tensors[]
| storage tensor | ename:VK_DESCRIPTOR_TYPE_TENSOR_ARM
endif::VK_ARM_tensors[]
|====
...
| sampled image
        | code:UniformConstant
ifdef::VK_QCOM_tile_shading[or code:TileAttachmentQCOM]
        | code:OpTypeImage (code:Sampled=1)|
| storage image
        | code:UniformConstant
ifdef::VK_QCOM_tile_shading[or code:TileAttachmentQCOM]
        | code:OpTypeImage (code:Sampled=2) |
...
| input attachment
        | code:UniformConstant
ifdef::VK_QCOM_tile_shading[or code:TileAttachmentQCOM]
        | code:OpTypeImage (code:Dim=code:SubpassData, code:Sampled=2)
...
| sample weight image
        | code:UniformConstant
ifdef::VK_QCOM_tile_shading[or code:TileAttachmentQCOM]
        | code:OpTypeImage (code:Depth=0, code:Dim=code:2D, +
...
ifdef::VK_ARM_tensors[]
| storage tensor
        | code:UniformConstant | code:OpTypeTensorARM |
endif::VK_ARM_tensors[]
|====
...
<<descriptorheaps-bindings, shader binding>>.
endif::VK_EXT_descriptor_heap[]
...
or a shader is created with ename:VK_SHADER_CREATE_DESCRIPTOR_HEAP_BIT_EXT,
endif::VK_EXT_shader_object[]
code:DescriptorSet decorations can: be any 32-bit unsigned integer value.
endif::VK_EXT_descriptor_heap[]
code:Binding decoration values can: be any 32-bit unsigned integer value.
...
The decorated array must: have an code:Element code:Type corresponding to a
...
<<descriptors-sets-updates,operations>>, if a resource variable is not an
array, it is treated as if it has an arrayElement of zero.

ifdef::VK_EXT_descriptor_heap[]
[NOTE]
====
Even though an array is assigned to a single code:Binding, when using
<<descriptorheaps-bindings, descriptor heap mappings>>, each element will be
...
ifdef::VK_EXT_descriptor_heap[]
These limits only apply to resources accessed with code:DescriptorSet and
code:Binding values.
endif::VK_EXT_descriptor_heap[]
...
ifdef::VK_EXT_descriptor_heap[]
Similarly, <<descriptorheaps-bindings, descriptor heap bindings>> may:
...
ifdef::VK_EXT_descriptor_heap[]
<<descriptorheaps-bindings, heap bindings>> must: specify a mapping for it
...
<<interfaces-resources-descriptorheap, descriptor heap interface>> do not
...
https://registry.khronos.org/spir-v/specs/unified1/SPIRV.html#ExplicitLayout[explicit
layout], it must: be laid out according to the following additional
requirements.
...
recursively as follows:
  ...
  * A matrix type inherits _scalar alignment_ from the equivalent array
    declaration.
ifdef::VK_EXT_descriptor_heap[]
  * code:OpTypeImage has a scalar alignment equal to the value of
    <<limits-imageDescriptorAlignment,pname:imageDescriptorAlignment>>
  * code:OpTypeBufferEXT has a scalar alignment equal to the value of
    <<limits-bufferDescriptorAlignment,pname:bufferDescriptorAlignment>>
  * code:OpTypeSampler has a scalar alignment equal to the value of
    <<limits-samplerDescriptorAlignment,pname:samplerDescriptorAlignment>>
ifdef::VK_ARM_tensors[]
  * code:OpTypeTensorARM has a scalar alignment equal to the value of
    <<limits-tensorDescriptorAlignment,pname:tensorDescriptorAlignment>>
endif::VK_ARM_tensors[]
endif::VK_EXT_descriptor_heap[]

ifdef::VK_EXT_descriptor_heap[]
The aligned size of a code:OpTypeImage, code:OpTypeBufferEXT, or
code:OpTypeSampler can be queried from within SPIR-V using
code:OpConstantSizeOfEXT, which can be used with the code:OffsetIdEXT or
code:ArrayStrideIdEXT decorations to lay out types in a descriptor heap.
code:OpConstantSizeOfEXT returns the following for values for each type:

[[interfaces-descriptor-sizes]]
.Shader Descriptor Sizes
[width="90%",cols="6,20",options="header"]
|====
| Type | Size
| code:OpTypeSampler | pname:samplerDescriptorSize aligned to pname:samplerDescriptorAlignment
| code:OpTypeImage | pname:imageDescriptorSize aligned to pname:imageDescriptorAlignment
| code:OpTypeBufferEXT | pname:bufferDescriptorSize aligned to pname:bufferDescriptorAlignment
ifdef::VK_KHR_acceleration_structure,VK_NV_ray_tracing[]
| code:OpTypeAccelerationStructureKHR | pname:bufferDescriptorSize aligned to pname:bufferDescriptorAlignment
...
recursively as follows:
  ...
  * A structure has a base alignment equal to the largest base alignment of
    ...
    SPIR-V module.
    (e.g., for a 1 byte aligned empty structure in the code:StorageBuffer
...
ifdef::VK_COMPUTE_VERSION_1_1,VK_KHR_relaxed_block_layout[]
...
endif::VK_COMPUTE_VERSION_1_1,VK_KHR_relaxed_block_layout[]
...
ifdef::VK_COMPUTE_VERSION_1_2,VK_EXT_scalar_block_layout[]
...
ifdef::VK_KHR_ray_tracing_pipeline[]
    ...
    or code:PushConstant,
ifdef::VK_EXT_descriptor_heap[]
    or the storage class is code:UniformConstant and the type is decorated
    with either code:SamplerHeapEXT or code:ResourceHeapEXT,
endif::VK_EXT_descriptor_heap[]
    then every member must: be aligned according to its scalar alignment.
endif::VK_COMPUTE_VERSION_1_2,VK_EXT_scalar_block_layout[]
...
ifdef::VK_COMPUTE_VERSION_1_2,VK_KHR_uniform_buffer_standard_layout[]
  . If the <<features-uniformBufferStandardLayout,
    pname:uniformBufferStandardLayout>> feature is not enabled, then any
endif::VK_COMPUTE_VERSION_1_2,VK_KHR_uniform_buffer_standard_layout[]
ifndef::VK_COMPUTE_VERSION_1_2,VK_KHR_uniform_buffer_standard_layout[]
  . Any
endif::VK_COMPUTE_VERSION_1_2,VK_KHR_uniform_buffer_standard_layout[]
...
ifdef::VK_EXT_descriptor_heap[]
  ...
  * Any code:ArrayStride,
ifdef::VK_EXT_descriptor_heap[]
    or code:ArrayStrideIdEXT,
endif::VK_EXT_descriptor_heap[]
    or code:MatrixStride decoration must: be a multiple of the alignment of
    the array or matrix as defined above.

ifdef::VK_COMPUTE_VERSION_1_2,VK_EXT_scalar_block_layout,VK_KHR_workgroup_memory_explicit_layout[]
...
ifdef::VK_KHR_workgroup_memory_explicit_layout[]
  * The storage class is code:Workgroup, and either the structure member is
    not part of a code:Block or the
...
endif::VK_COMPUTE_VERSION_1_2,VK_EXT_scalar_block_layout,VK_KHR_workgroup_memory_explicit_layout[]
...
Any two variables declared in the code:Input storage class listed as
operands on the same code:OpEntryPoint must: not have the same code:BuiltIn
decoration.
Any two variables declared in the code:Output storage class listed as
operands on the same code:OpEntryPoint must: not have the same code:BuiltIn
decoration.
...
code:Offset, code:OffsetIdEXT, code:ArrayStride, code:ArrayStrideIdEXT, and
code:MatrixStride decorations as specified in <<interfaces-resources-layout,
Offset and Stride Assignment>>.
endif::VK_EXT_descriptor_heap[]
...
ifdef::VK_GRAPHICS_VERSION_1_1,VK_KHR_shader_draw_parameters[]
...
****
--
endif::VK_GRAPHICS_VERSION_1_1,VK_KHR_shader_draw_parameters[]
...
****
  ...
  * [[VUID-{refpage}-CullPrimitiveEXT-07036]]
    code:CullPrimitiveEXT must: decorate a scalar boolean member of a
    structure decorated as code:Block, or decorate a variable of type
    code:OpTypeArray of boolean values
  * [[VUID-{refpage}-CullPrimitiveEXT-10589]]
    If code:CullPrimitiveEXT is declared as an array of boolean values, the
    size of the array must: match the value specified by
    code:OutputPrimitivesEXT
  * [[VUID-{refpage}-CullPrimitiveEXT-10590]]
    If code:CullPrimitiveEXT decorates a member of a structure, the variable
    declaration of the containing code:Block type must: have an array size
    that matches the value specified by code:OutputPrimitivesEXT
  * [[VUID-{refpage}-CullPrimitiveEXT-10591]]
...
[NOTE]
====
When used in a custom resolve operation and the <<features-customResolve,
pname:customResolve>> feature is enabled, the dimensions in pixels returned
may: be [eq]#(1,1)# if the fragment area was reduced.
====
+
...
****
  ...
  * [[VUID-{refpage}-None-08751]]
    ...
    ename:VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_DATA_ACCESS_BIT_KHR
****
--
...
In a
ifdef::VK_GRAPHICS_VERSION_1_2,VK_EXT_shader_viewport_index_layer,VK_NV_viewport_array2[]
ifdef::VK_NV_mesh_shader,VK_EXT_mesh_shader[mesh,]
vertex, tessellation evaluation, or
endif::VK_GRAPHICS_VERSION_1_2,VK_EXT_shader_viewport_index_layer,VK_NV_viewport_array2[]
...
ifdef::VK_GRAPHICS_VERSION_1_2[]
  ...
  * [[VUID-{refpage}-Layer-07039]]
    ...
    decoration
  * [[VUID-{refpage}-Layer-10592]]
    code:Layer within the code:MeshEXT {ExecutionModel} must: decorate a
    scalar 32-bit integer member of a structure decorated as code:Block, or
    decorate a variable of type code:OpTypeArray of scalar 32-bit integer
    values
  * [[VUID-{refpage}-Layer-10593]]
    If code:Layer is declared as an array of 32-bit integer values, within
    the code:MeshEXT {ExecutionModel}, size of the array must: match the
    value specified by code:OutputPrimitivesEXT
  * [[VUID-{refpage}-Layer-10594]]
    If code:Layer decorates a member of a structure, the variable
    declaration of the containing code:Block type must: have an array size
    that matches the value specified by code:OutputPrimitivesEXT
****
--
...
code:MeshViewCountNV are poison.
...
then the variable decorated with code:PointCoord contains poison.
...
ifdef::VK_GRAPHICS_VERSION_1_0,VK_NV_mesh_shader,VK_EXT_mesh_shader,VK_NV_ray_tracing,VK_KHR_ray_tracing_pipeline[]
...
****
  ...
  * [[VUID-{refpage}-PrimitiveId-07040]]
    ...
    {ExecutionModel} must: also be decorated with the code:PerPrimitiveEXT
    decoration
  * [[VUID-{refpage}-PrimitiveId-10595]]
    code:PrimitiveId within the code:MeshEXT {ExecutionModel} must: decorate
    a scalar 32-bit integer member of a structure decorated as code:Block,
    or decorate a variable of type code:OpTypeArray of 32-bit integer values
  * [[VUID-{refpage}-PrimitiveId-10596]]
  ...
  * [[VUID-{refpage}-PrimitiveId-10597]]
    If code:PrimitiveId decorates a member of a structure, the variable
    declaration of the containing code:Block type must: have an array size
    that matches the value specified by code:OutputPrimitivesEXT
****
--
endif::VK_GRAPHICS_VERSION_1_0,VK_NV_mesh_shader,VK_EXT_mesh_shader,VK_NV_ray_tracing,VK_KHR_ray_tracing_pipeline[]
...
****
  ...
  * [[VUID-{refpage}-PrimitivePointIndicesEXT-07045]]
    ...
    where [eq]#N# is the value specified by the "`Primitive Count`" operand
    of code:OpSetMeshOutputsEXT
  * [[VUID-{refpage}-PrimitivePointIndicesEXT-12335]]
    All index values written to the array decorated with
    code:PrimitivePointIndicesEXT must: be in the range [eq]#[0, N-1]#,
    where [eq]#N# is the value specified by the "`Vertex Count`" operand of
    code:OpSetMeshOutputsEXT
...
****
  ...
  * [[VUID-{refpage}-PrimitiveLineIndicesEXT-07051]]
    ...
    [eq]#N# is the value specified by the "`Primitive Count`" operand of
    code:OpSetMeshOutputsEXT
  * [[VUID-{refpage}-PrimitiveLineIndicesEXT-12336]]
    All index values written to the array decorated with
    code:PrimitiveLineIndicesEXT must: be in the range [eq]#[0, N-1]#, where
    [eq]#N# is the value specified by the "`Vertex Count`" operand of
    code:OpSetMeshOutputsEXT
...
****
  ...
  * [[VUID-{refpage}-PrimitiveTriangleIndicesEXT-07057]]
    ...
    where [eq]#N# is the value specified by the "`Primitive Count`" operand
    of code:OpSetMeshOutputsEXT
  * [[VUID-{refpage}-PrimitiveTriangleIndicesEXT-12337]]
    All index values written to the array decorated with
    code:PrimitiveTriangleIndicesEXT must: be in the range [eq]#[0, N-1]#,
    where [eq]#N# is the value specified by the "`Vertex Count`" operand of
    code:OpSetMeshOutputsEXT
...
****
  ...
  * [[VUID-{refpage}-PrimitiveShadingRateKHR-07059]]
    ...
    code:MeshEXT {ExecutionModel} must: also be decorated with the
    code:PerPrimitiveEXT decoration
  * [[VUID-{refpage}-PrimitiveShadingRateKHR-10598]]
    code:PrimitiveShadingRateKHR within the code:MeshEXT {ExecutionModel}
    must: decorate a scalar 32-bit integer member of a structure decorated
    as code:Block, or decorate a variable of type code:OpTypeArray of 32-bit
    integer values
  * [[VUID-{refpage}-PrimitiveShadingRateKHR-10599]]
    If code:PrimitiveShadingRateKHR is declared as an array of 32-bit
    integer values, within the code:MeshEXT {ExecutionModel}, size of the
    array must: match the value specified by code:OutputPrimitivesEXT
  * [[VUID-{refpage}-PrimitiveShadingRateKHR-10600]]
    If code:PrimitiveShadingRateKHR decorates a member of a structure, the
    variable declaration of the containing code:Block type must: have an
    array size that matches the value specified by code:OutputPrimitivesEXT
  * [[VUID-{refpage}-PrimitiveShadingRateKHR-12275]]
    If code:PrimitiveShadingRateKHR is declared in the code:MeshEXT
    {ExecutionModel}, the <<features-primitiveFragmentShadingRateMeshShader,
    primitiveFragmentShadingRateMeshShader>> feature must: be enabled
****
--
...
ifdef::VK_COMPUTE_VERSION_1_1,VK_EXT_shader_subgroup_ballot[]
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_EXT_subgroup_size_control[]
If the shader was created with <<shaders-full-subgroups, full subgroups>>,
...
endif::VK_COMPUTE_VERSION_1_3,VK_EXT_subgroup_size_control[]
====
...
+
ifdef::VK_COMPUTE_VERSION_1_3,VK_EXT_subgroup_size_control[]
If the shader was created with <<shaders-varying-subgroup-size, varying
subgroup size allowed>>, the code:SubgroupSize decorated variable will
...
subgroup scope>>.
+
If the shader was created with a <<shaders-required-subgroup-size, required
subgroup size>>, the code:SubgroupSize decorated variable will match that
value.
+
If the shader does not allow <<shaders-varying-subgroup-size, varying
subgroup sizes>>, the
endif::VK_COMPUTE_VERSION_1_3,VK_EXT_subgroup_size_control[]
ifndef::VK_COMPUTE_VERSION_1_3,VK_EXT_subgroup_size_control[]
The
endif::VK_COMPUTE_VERSION_1_3,VK_EXT_subgroup_size_control[]
...
****
--
endif::VK_COMPUTE_VERSION_1_1,VK_EXT_shader_subgroup_ballot[]
...
ifdef::VK_QCOM_tile_shading[]
[[interfaces-builtin-variables-tileoffset]]
[open,refpage='TileOffsetQCOM',desc='Tile offset of a shader invocation',type='builtins']
--
:refpage: TileOffsetQCOM

code:TileOffsetQCOM::

The code:TileOffsetQCOM decoration can: be applied to a shader input which
will be filled with the framebuffer coordinates of the active tile.
+
When <<renderpass-tile-shading, per-tile execution model>> is enabled, for
the current shader invocation [eq]#x# and [eq]#y# components of
code:TileOffsetQCOM reflect the framebuffer integer coordinates of the
top-left texel of the tile corresponding to the shader invocation.
+
Otherwise, the [eq]#x# and [eq]#y# components of code:TileOffsetQCOM are
filled with [eq]#(0,0)#.

.Valid Usage
****
  * [[VUID-{refpage}-TileOffsetQCOM-10626]]
    The code:TileOffsetQCOM decoration must: be used only within the
    code:Fragment {ExecutionModel} or code:GLCompute {ExecutionModel}
  * [[VUID-{refpage}-TileOffsetQCOM-10627]]
    The variable decorated with code:TileOffsetQCOM must: be declared using
    the code:Input {StorageClass}
  * [[VUID-{refpage}-TileOffsetQCOM-10628]]
    The variable decorated with code:TileOffsetQCOM must: be declared as a
    two-component vector of 32-bit integer values
****

--

[[interfaces-builtin-variables-tilesize]]
[open,refpage='TileDimensionQCOM',desc='Tile offset of a shader invocation',type='builtins']
--
:refpage: TileDimensionQCOM

code:TileDimensionQCOM::

The code:TileDimensionQCOM decoration can: be applied to a shader input
which will be filled with the width and height of the active tile.
+
When <<renderpass-tile-shading, per-tile execution model>> is enabled, for
the current shader invocation [eq]#x# and [eq]#y# components of
code:TileDimensionQCOM reflect the with and height of the tile corresponding
to the shader invocation.
+
Otherwise, the [eq]#x# and [eq]#y# components of code:TileDimensionQCOM are
filled with [eq]#(0,0)#.

.Valid Usage
****
  * [[VUID-{refpage}-TileDimensionQCOM-10629]]
    The code:TileDimensionQCOM decoration must: be used only within the
    code:Fragment {ExecutionModel} or code:GLCompute {ExecutionModel}
  * [[VUID-{refpage}-TileDimensionQCOM-10630]]
    The variable decorated with code:TileDimensionQCOM must: be declared
    using the code:Input {StorageClass}
  * [[VUID-{refpage}-TileDimensionQCOM-10631]]
    The variable decorated with code:TileDimensionQCOM must: be declared as
    a two-component vector of 32-bit integer values
****

--

[[interfaces-builtin-variables-tileapronsize]]
[open,refpage='TileApronSizeQCOM',desc='Tile apron size of a shader invocation',type='builtins']
--
:refpage: TileApronSizeQCOM

code:TileApronSizeQCOM::

The code:TileApronSizeQCOM decoration can: be applied to a shader input
which will be filled with the width and height of the active tile's apron.
+
If <<renderpass-tile-shading, per-tile execution model>> is enabled for the
current shader invocation, and is executing in a dynamic render pass or is
executing in a subpass where
ename:VK_SUBPASS_DESCRIPTION_TILE_SHADING_APRON_BIT_QCOM is included in its
pname:flags, [eq]#x# and [eq]#y# components of code:TileApronSizeQCOM
reflect the with and height of the tile apron corresponding to the shader
invocation.
+
Otherwise, the [eq]#x# and [eq]#y# components of code:TileApronSizeQCOM are
filled with [eq]#(0,0)#.

.Valid Usage
****
  * [[VUID-{refpage}-TileApronSizeQCOM-10632]]
    The code:TileApronSizeQCOM decoration must: be used only within the
    code:Fragment {ExecutionModel} or code:GLCompute {ExecutionModel}
  * [[VUID-{refpage}-TileApronSizeQCOM-10633]]
    The variable decorated with code:TileApronSizeQCOM must: be declared
    using the code:Input {StorageClass}
  * [[VUID-{refpage}-TileApronSizeQCOM-10634]]
    The variable decorated with code:TileApronSizeQCOM must: be declared as
    a two-component vector of 32-bit integer values
****
...
of code:ViewportIndex in the fragment shader is poison.
endif::VK_QCOM_multiview_per_view_viewports[]
...
ifdef::VK_GRAPHICS_VERSION_1_2[]
  ...
  * [[VUID-{refpage}-ViewportIndex-07060]]
    ...
    {ExecutionModel} must: also be decorated with the code:PerPrimitiveEXT
    decoration
  * [[VUID-{refpage}-ViewportIndex-10601]]
    code:ViewportIndex within the code:MeshEXT {ExecutionModel} must:
    decorate a scalar 32-bit integer member of a structure decorated as
    code:Block, or decorate a variable of type code:OpTypeArray of scalar
    32-bit integer values
  * [[VUID-{refpage}-ViewportIndex-10602]]
    If code:ViewportIndex is declared as an array of 32-bit integer values,
    within the code:MeshEXT {ExecutionModel}, size of the array must: match
    the value specified by code:OutputPrimitivesEXT
  * [[VUID-{refpage}-ViewportIndex-10603]]
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_maintenance4[or code:LocalSizeId]
execution mode.
...
****
  ...
  * [[VUID-{refpage}-WorkgroupSize-04427]]
    ...
    three-component vector of 32-bit integer values
ifdef::VK_QCOM_tile_shading[]
  * [[VUID-{refpage}-TileShadingRateQCOM-10635]]
    If the code:TileShadingRateQCOM code:Execution code:Mode is used,
    variables decorated with code:WorkgroupSize must: be declared using the
    code:Input {StorageClass}
endif::VK_QCOM_tile_shading[]
****
--
...
[open,refpage='CoreMaxIDARM',desc='Max core ID that can be observed on the device running the invocation reading CoreMaxIDARM',type='builtins']
--
...
[open,refpage='WarpMaxIDARM',desc='Max ID for a warp on the core running a shader invocation',type='builtins']
--
