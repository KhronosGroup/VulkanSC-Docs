...
ifdef::VK_EXT_mesh_shader[]
  ...
  * [[VUID-VkQueryPoolCreateInfo-queryType-09534]]
    If pname:queryType is ename:VK_QUERY_TYPE_PIPELINE_STATISTICS,
    pname:pipelineStatistics must: not be zero
ifdef::VK_KHR_performance_query[]
...
include::{generated}/api/enums/VkQueryResultStatusKHR.adoc[]
  ...
  * ename:VK_QUERY_RESULT_STATUS_NOT_READY_KHR indicates that the query
    result is not yet available.
  * ename:VK_QUERY_RESULT_STATUS_ERROR_KHR indicates that operations did not
...
<<synchronization-dependencies-execution, happens-after>> another must: not
have a lower value unless its value overflows the maximum supported integer
bit width of the query.
Timestamps written by this command must: be in the
ifdef::VK_KHR_calibrated_timestamps[ename:VK_TIME_DOMAIN_DEVICE_KHR]
ifndef::VK_KHR_calibrated_timestamps[ename:VK_TIME_DOMAIN_DEVICE_EXT]
<<VkTimeDomainKHR, time domain>>.
...
include::{generated}/api/enums/VkVideoEncodeFeedbackFlagBitsKHR.adoc[]
  ...
  * ename:VK_VIDEO_ENCODE_FEEDBACK_BITSTREAM_BUFFER_OFFSET_BIT_KHR specifies
    ...
    <<video-encode-commands,video encode command>>, this value will always
    be zero, meaning that bitstream data is always written to the buffer
    specified in slink:VkVideoEncodeInfoKHR::pname:dstBuffer starting from
  ...
  * ename:VK_VIDEO_ENCODE_FEEDBACK_BITSTREAM_HAS_OVERRIDES_BIT_KHR specifies
    ...
    slink:VkVideoEncodeInfoKHR::pname:dstBuffer contains
    <<encode-overrides,overridden parameters>>.
...
ename:VK_QUERY_RESULT_WITH_STATUS_BIT_KHR when retrieving the results of
video encode feedback queries and ignore such undefined: video encode
feedback values for any <<encode-unsuccessful,unsuccessfully>> completed
