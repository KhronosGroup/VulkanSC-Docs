...
include::{generated}/api/enums/VkMemoryPropertyFlagBits.adoc[]
  ...
  * ename:VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT bit specifies that memory
    ...
    flink:vkMapMemory.
  * [[memory-coherent]] ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT bit
    specifies that the host cache management commands
    flink:vkFlushMappedMemoryRanges and flink:vkInvalidateMappedMemoryRanges
...
[[host-memory-import-non-visible-type]]
Implementations may: support importing host pointers for memory types which
are not host-visible.
In this case, after a successful call to flink:vkAllocateMemory, the memory
range imported from pname:pHostPointer must: not be accessed by the
application until the slink:VkDeviceMemory has been destroyed.
Memory contents for the host memory becomes undefined: on import, and is
left undefined: after the slink:VkDeviceMemory has been destroyed.
Applications must: also not access host memory which is mapped to the same
physical memory as pname:pHostPointer, but mapped to a different host
pointer while the slink:VkDeviceMemory handle is valid.
...
The value returned by pname:memoryTypeBits should: only include bits that
...
<<host-memory-import-non-visible-type,slink:VkImportMemoryHostPointerInfoEXT>>.
