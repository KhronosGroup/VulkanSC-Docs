...
specification.
Vulkan SC was designed to maintain as much compatibility as possible with
base Vulkan.
Some functionality has been removed, some new functionality has been added,
but where the APIs overlap the only changes are to Valid Usage rules.
All functions and structures that exist in both APIs have the same
signatures, and enums and defined constants that exist in both APIs have the
same values.
